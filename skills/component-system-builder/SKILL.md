...
description: Stand up a closed-world component and agent-rules system for a platform (Electron, Astro, SwiftUI), or retrofit one onto an existing project by extracting a component kit, tokens, a pattern table and rules from code that grew without them. Use when AI-generated apps on one platform need to come out consistent. Consistency comes from removing choices.
---
...
- `references/runtime-discipline.md`: the studied reference's "muscles": pattern-skill
  ...
  theming/compositing lore. Read when writing artifacts 2 to 6 (§7 BEFORE
...
Before building rungs 4 to 6, draft the **enforcement map** (which mechanism

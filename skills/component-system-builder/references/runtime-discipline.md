...
apps *consistent*; these runtime behaviors are where the studied reference
implementation's apps got *correct* and *cheap*. Derived from that reference's
orchestrator prompt and skill bodies.

## 1. Pattern-skill anatomy (the table is only a third of it)

The studied reference's component-patterns skill is ~14KB; the gate + table are the smallest
...
The studied reference's orchestrator defaults to NO delegation: sub-agents
start cache-cold, must re-gather context, and often cost more than direct
work. Delegate only when a condition list is met (genuinely parallel
workstreams, or a large isolated single-layer task), if any condition is
missing, do the work directly, and do not re-test that decision later. Handoff
mechanics when you DO delegate:
...
Invoke the relevant skill immediately BEFORE the work it governs, not all skills up
...
on demand. Honest note: in the studied reference the harness wrote HISTORY and ran a
...
- **Guide line-number index.** A 50KB recipe guide is affordable only with a
  section index ("Critical Rules: lines 52 to 72, use as Read offset; if
...
Capture how the platform actually composites before authoring seeds. The
studied reference's example: light-mode `--bg` rendered at 40% opacity over native window

...
5. **Launch the advisor first** (so it exists before any worker can finish and call back):
   - `herdr agent start "<advisor_name>" --kind claude --pane "<advisor_pane>" -- <advisor.model_flag words> --permission-mode auto`. It returns once the TUI is ready (default timeout 30s; raise with `--timeout <ms>` if needed). CAUTION: "ready" can also mean claude is showing a startup dialog, so always run the dismissal check next.
   - **Dismiss startup dialogs:** `herdr pane read "<advisor_pane>" --source visible --lines 20`. If it shows the folder-trust dialog ("Do you trust the files in this folder?"), `herdr pane send-keys "<advisor_pane>" Enter` to accept; claude blocks on this in any directory it has not seen before (fresh dated dirs, scratchpads, new worktree checkouts), and a prompt sent while it is up stalls. If it shows "new MCP servers found", `herdr pane send-keys "<advisor_pane>" Esc`. Re-read after each keypress to confirm the TUI is at the normal input box.
...
6. **Launch each worker** in its pane:
   ...
   - Dismiss startup dialogs as in step 5 (trust dialog especially: workers routinely start in fresh directories and worktree checkouts).
...
- Run ONE assessment pass now: `herdr agent list` gives every worker's `agent_status` and live `terminal_title` (the title describes what that agent is currently doing). Cross-check it against the roster: a roster worker MISSING from `agent list` is not gone, its detection may just lag (observed with fresh worktree panes), so `herdr pane read` its pane directly. Triage from the list first; only `herdr pane read <pane_id> --source recent --lines 40` the workers that look stuck, off-theme, or missing from the list. Post a short roll-up in THIS pane (who is doing what, who is stuck).
